...
def build_autoencoder(input_shape):
    ...
    x = UpSampling2D((2, 2))(x)
    outputs = Conv2D(input_shape[2], (3, 3), activation='sigmoid', padding='same')(x)
...
def train_autoencoder(real_videos, deepfake_videos):
    ...
    x_train, x_val, _, _ = train_test_split(np.concatenate((real_frames, deepfake_frames)), np.zeros(len(real_frames) + len(deepfake_frames)), test_size=0.2)
    ...
    autoencoder.fit(x_train, x_train, validation_data=(x_val, x_val), epochs=10, batch_size=32)
...
def train_discriminator(encoder, real_frames, deepfake_frames):
    ...
    labels = np.concatenate((np.ones(len(real_encoded)), np.zeros(len(deepfake_encoded))))
    ...
    discriminator.fit(x_train, y_train, validation_data=(x_val, y_val), epochs=10, batch_size=32)
...
train_real_videos, test_real_videos = train_test_split(real_videos, test_size=0.2)
train_deepfake_videos, test_deepfake_videos = train_test_split(deepfake_videos, test_size=0.2)

# Train the autoencoder
encoder, decoder = train_autoencoder(train_real_videos, train_deepfake_videos)

# Train the discriminator
discriminator = train_discriminator(encoder, train_real_videos, train_deepfake_videos)

# Evaluate the model
accuracy, precision, recall, f1 = evaluate_model(encoder, discriminator, test_real_videos, test_deepfake_videos)

# Deploy the deepfake detection system
input_video = cv2.VideoCapture('input_video.mp4')  # Update with your video file
